...
def run_single_rollout(agent, task, selected_id, traj_idx, META_CONFIG, INSTRUCTIONS, save_video=True):
    ...
    try:
        ...
        # Generate comparison image if prediction visualization is enabled
        if visualize_prediction and len(actual_frames) > 0 and len(predicted_frames) > 0 and len(executed_frames) > 0:
            save_comparison_image(actual_frames, predicted_frames, executed_frames, task, traj_idx, META_CONFIG)
