from typing import Tuple
from dataclasses import dataclass
import numpy as np
from multiprocessing.managers import SharedMemoryManager

# Note: atomics package is optional, fallback to simple implementation if not available
try:
    from atomics import atomicview, MemoryOrder, UINT
    ATOMICS_AVAILABLE = True
except ImportError:
    ATOMICS_AVAILABLE = False
    import threading

@dataclass
class ArraySpec:
    name: str
    shape: Tuple[int]
    dtype: np.dtype


class SharedAtomicCounter:
    def __init__(self, 
            shm_manager: SharedMemoryManager, 
            size: int = 8  # 64bit int
            ):
        shm = shm_manager.SharedMemory(size=size)
        self.shm = shm
        self.size = size
        self._lock = threading.Lock() if not ATOMICS_AVAILABLE else None
        self.store(0)  # initialize

    @property
    def buf(self):
        return self.shm.buf[:self.size]

    def load(self) -> int:
        if ATOMICS_AVAILABLE:
            with atomicview(buffer=self.buf, atype=UINT) as a: 
                value = a.load(order=MemoryOrder.ACQUIRE)
            return value
        else:
            # Fallback implementation using threading lock
            with self._lock:
                return int.from_bytes(self.buf, byteorder='little')
    
    def store(self, value: int):
        if ATOMICS_AVAILABLE:
            with atomicview(buffer=self.buf, atype=UINT) as a:
                a.store(value, order=MemoryOrder.RELEASE)
        else:
            # Fallback implementation
            with self._lock:
                self.buf[:] = value.to_bytes(self.size, byteorder='little')
    
    def add(self, value: int):
        if ATOMICS_AVAILABLE:
            with atomicview(buffer=self.buf, atype=UINT) as a:
                a.add(value, order=MemoryOrder.ACQ_REL)
        else:
            # Fallback implementation
            with self._lock:
                current = int.from_bytes(self.buf, byteorder='little')
                new_value = current + value
                self.buf[:] = new_value.to_bytes(self.size, byteorder='little')