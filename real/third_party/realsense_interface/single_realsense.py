...
class SingleRealsense(mp.Process):
    ...
    def run(self):
        ...
        try:
            ...
            # 等待相机稳定，丢弃前几帧
            if self.verbose:
                print(f'[SingleRealsense {self.serial_number}] Waiting for camera to stabilize...')
            for _ in range(10):  # 丢弃前10帧
                try:
                    pipeline.wait_for_frames(timeout_ms=10000)  # 增加超时到10秒
                except RuntimeError:
                    if self.verbose:
                        print(f'[SingleRealsense {self.serial_number}] Warning: Frame timeout during warmup')
            
            if self.verbose:
                print(f'[SingleRealsense {self.serial_number}] Camera stabilized, starting main loop.')
            
            while not self.stop_event.is_set():
                # wait for frames to come in
                frameset = pipeline.wait_for_frames(timeout_ms=10000)  # 增加超时到10秒
